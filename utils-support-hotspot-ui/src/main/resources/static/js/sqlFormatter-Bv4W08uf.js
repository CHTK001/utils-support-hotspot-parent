...
const reservedSelect$i = expandPhrases(["SELECT [ALL | DISTINCT] [AS STRUCT | AS VALUE]"]);
...
const bigquery = {
  ...
  tokenizerOptions: {
    ...
    reservedClauses: [...reservedClauses$i, ...tabularOnelineClauses$h, ...standardOnelineClauses$h],
...
const escapeRegExp = (string2) => string2.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&");
...
const lineComment = (lineCommentTypes) => new RegExp(`(?:${lineCommentTypes.map(escapeRegExp).join("|")}).*?(?=\r
|\r|
|$)`, "uy");
...
const variable = (varTypes) => patternToRegex(varTypes.map((varType) => "regex" in varType ? varType.regex : singleQuotePattern(varType)).join("|"));
...
function createParser(tokenizer) {
  ...
  const parser = new NearleyParser(Grammar.fromCompiled(grammar), { lexer: lexer2 });
  return {
